...
export function AdminMembersPage() {
	...
	const plans = useQuery({ queryKey: ['admin', user?.id, 'plans'], queryFn: planBillingApi.plans, enabled: user?.role === 'admin' });
	...
	const invalidate = () => void queryClient.invalidateQueries({ queryKey: ['admin', user?.id] });
	const createUser = useMutation({
		mutationFn: adminApi.createUser,
		onSuccess: () => { setNewUsername(''); setNewPassword(''); setNewPlanId('plan_basic'); setBonusTokens('0'); invalidate(); },
	});
	const updateUser = useMutation({ mutationFn: ({ userId, status }: { userId: string; status: 'active' | 'banned' }) => adminApi.updateUser(userId, { status }), onSuccess: invalidate });
	const deleteUser = useMutation({ mutationFn: ({ userId, reason }: { userId: string; reason: string }) => adminApi.deleteUser(userId, reason), onSuccess: invalidate });
	const resetPassword = useMutation({
		...
		onSuccess: (data) => { window.alert(`${t('admin.temporaryPassword')}: ${data.temporary_password}\n${t('admin.expiresAt')}: ${data.expires_at}`); invalidate(); },
	});
	const revokeSessions = useMutation({ mutationFn: adminApi.revokeSessions, onSuccess: invalidate });
	...
	return (
		<>
			<AdminHeader title={t('admin.nav.members')} description={t('admin.membersDescription')} onRefresh={invalidate} loading={users.isFetching} />
			...
			<Card>
				...
				<CardContent className="space-y-5">
					<form onSubmit={submitCreate} className="grid gap-3 rounded-lg bg-muted/40 p-3 md:grid-cols-5">
						<div className="space-y-1.5"><Label htmlFor="admin-new-username">{t('admin.username')}</Label><Input id="admin-new-username" value={newUsername} onChange={(e) => setNewUsername(e.target.value)} required /></div>
						<div className="space-y-1.5"><Label htmlFor="admin-new-password">{t('admin.initialPassword')}</Label><Input id="admin-new-password" type="password" minLength={8} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required /></div>
					...
					<div className="flex flex-wrap items-center gap-3"><Input value={keyword} onChange={(e) => { setKeyword(e.target.value); reset(); }} placeholder={t('admin.searchMembers')} className="max-w-xs" /><select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); reset(); }} className="border-input bg-background h-9 rounded-md border px-3 text-sm"><option value="">{t('admin.allStatuses')}</option><option value="active">{t('admin.statusValues.active')}</option><option value="locked">{t('admin.statusValues.locked')}</option><option value="banned">{t('admin.statusValues.banned')}</option><option value="deleted">{t('admin.statusValues.deleted')}</option></select>{users.isFetching && <Loader2 className="size-4 animate-spin text-muted-foreground" />}</div>
					<div className="overflow-x-auto rounded-lg border"><table className="w-full text-sm"><thead className="border-b bg-muted/40"><tr><th className="px-3 py-2 text-left font-medium">{t('admin.username')}</th><th className="px-3 py-2 text-left font-medium">{t('admin.status')}</th><th className="px-3 py-2 text-left font-medium">{t('admin.plan')}</th><th className="px-3 py-2 text-right font-medium">{t('admin.usage')}</th><th className="px-3 py-2 text-right font-medium">{t('admin.actions')}</th></tr></thead><tbody>
						{items.map((item) => <tr key={item.id} className="border-b last:border-0"><td className="px-3 py-2 font-medium">{item.username}</td><td className="px-3 py-2"><Badge variant={statusVariant(item.status)}>{t(`admin.statusValues.${item.status}`)}</Badge></td><td className="px-3 py-2 text-muted-foreground">{item.plan_name}</td><td className="px-3 py-2 text-right font-mono text-xs">{formatNumber(item.monthly_used)} / {formatNumber(item.monthly_quota)}</td><td className="px-3 py-2"><div className="flex flex-wrap justify-end gap-1">{item.role !== 'admin' && <><Button variant="ghost" size="sm" disabled={updateUser.isPending} onClick={() => updateUser.mutate({ userId: item.id, status: item.status === 'active' ? 'banned' : 'active' })}>{item.status === 'active' ? t('admin.ban') : t('admin.activate')}</Button><Button variant="destructive" size="sm" disabled={deleteUser.isPending} onClick={() => { if (!window.confirm(t('admin.deleteConfirm', { username: item.username }))) return; const reason = window.prompt(t('admin.deleteReasonPrompt')); if (reason && reason.trim().length >= 4) deleteUser.mutate({ userId: item.id, reason: reason.trim() }); else if (reason !== null) window.alert(t('admin.reasonRequired')); }}>{t('common.delete')}</Button><Button variant="ghost" size="sm" disabled={resetPassword.isPending} onClick={() => handleResetPassword(item)}><KeyRound />{t('admin.resetPassword')}</Button><Button variant="ghost" size="sm" disabled={revokeSessions.isPending} onClick={() => { if (window.confirm(t('admin.revokeSessionsConfirm', { username: item.username }))) revokeSessions.mutate(item.id); }}><UserRoundX />{t('admin.revokeSessions')}</Button></>}</div></td></tr>)}
